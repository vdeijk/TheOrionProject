...
namespace TurnBasedStrategy.Controllers
{
    ...
    public class ProjectileLaserController : ProjectileBaseController
    {
        ...
        public override void Setup(UnitSingleController unit, UnitSingleController targetUnit)
        {
            this.unit = unit;
            this.targetUnit = targetUnit;

            Vector3 startPosition = transform.position;
            Vector3 direction = (targetUnit.Data.TransformData[PartType.Torso].position - startPosition).normalized;
            float maxDistance = Vector3.Distance(startPosition, targetUnit.Data.TransformData[PartType.Torso].position) + 5f;
            Vector3 hitPoint;
            UnitSingleController hitUnit = null;
            ...
            if (Physics.Raycast(startPosition, direction, out RaycastHit hitInfo, maxDistance))
            {
                ...
                hitUnit = hitInfo.collider.GetComponent<UnitSingleController>();
            }
            else
            {
                hitPoint = targetUnit.Data.TransformData[PartType.Torso].position;
                targetPos = hitPoint;
            }
